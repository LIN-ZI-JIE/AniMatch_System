...
{
 $sql_query="SELECT * FROM message WHERE message_ID=".$_GET['edit_id'];
...
if(isset($_POST['btn-update']))
{
 ...
 $sql_query  = "SELECT * FROM message WHERE message_ID = $message_ID";
 $result_set = mysqli_query($link, $sql_query);
 //$row = mysqli_fetch_assoc($result_set);
 

 //sql query for update data into database
 $sql_t = "UPDATE message SET message_content =$message_content WHERE message_ID =$message_ID";
 ...
 if(mysqli_query($link, $sql_t))
 {
  ...
  alert('留言修改成功');
  window.location.href='adopter_chat.php';
  </script>
  <?php
 }
 else
	  ...
	  <div class="w3-card w3-round w3-white">
       <div class="w3-container w3-padding">
         ...
         <form  method="post" target="_self"> 
			...
			$sql2 = "SELECT * FROM message WHERE member_ID = $member_ID";
			$result2 = execute_sql($link, "animatch", $sql2);
			$row2 = mysqli_fetch_assoc($result2);
			//$message_ID1 = $row2['message_ID'];
			$sql3 = "SELECT * FROM message WHERE message_ID = $message_ID";
		    $result3 = execute_sql($link, "animatch", $sql3);
		    $row3 = mysqli_fetch_assoc($result3);
			?>
			
			<h5><?php echo $row3['message_content']; ?></h5>
			  			
			  <hr class="w3-clear">
			
			  <p><input class="w3-input w3-padding-16 w3-border" type="text" placeholder="修改留言.." required name="message_content"></p>
			  <p><button class="w3-button w3-right w3-theme" name="btn-update" a href="javascript:edit_id('<?php echo $row3['message_ID']; ?>')" type="submit">確認修改</button></p>
			</form>
